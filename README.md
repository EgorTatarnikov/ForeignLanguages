# Foreign Languages

В данном репозитории собраны мои проекты, инструменты и материалы для изучения иностранных языков.<br>
В процессе изучения английского языка я выработал собственный подход и разработал свои инструменты.<br>
Буду рад, если эти материалы принесут пользу и другим людям.

## 🎯 Цель репозитория

- Демонстрация проектов и навыков в создании готовых продуктов с использованием разных технологий
- Агрегация инструментов и материалов по изучению языков в одном месте
- Помощь другим людям в изучении иностранных языков

## 📦 Проекты

### <img align="center" src="https://raw.githubusercontent.com/EgorTatarnikov/WordByHeart/main/src/logo/WordByHeart_v3.svg" width="24" alt="wt badge"> [Word by Heart](https://github.com/EgorTatarnikov/WordByHeart)
Методика изучения английского языка через чтение книг и просмотр сериалов в оригинале.

В основе методики лежит **NLP-программа**, которая:
- анализирует текст
- выбирает **наиболее важные слова**
- строит **IPA-транскрипцию**
- переводит лексику **с учётом контекста**
- формирует таблицы и двусторонние карточки для изучения
  
Изучение небольшого количества наиболее важных слов позволяет быстро приступить к чтению и просмотру контента на языке оригинала.

### <img align="center" src="https://raw.githubusercontent.com/EgorTatarnikov/WordsTraining/main/assets/wt_badge.svg" width="24" alt="wt badge"> [Words Training](https://github.com/EgorTatarnikov/WordsTraining)


Desktop-приложение на **C++ Builder** для тренировки перевода и правописания английских слов.

### <img align="center" src="https://raw.githubusercontent.com/EgorTatarnikov/ForeignLanguages/main/Iterator/Assets/Sprites/iterator_r205.svg" width="24" alt="wt badge"> Iterator (Word Repeater)
Android-приложение на **Unity** для интервального повторения английских слов.

### 📒 Прочее
- **[Minilex](https://github.com/EgorTatarnikov/ForeignLanguages/blob/main/Minilex_24-05-10.xlsx)** – минимальный набор слов разных языков, позволяющий понимать основной смысл разговорной речи. Составляется и дополняется мной, возможны ошибки.
- **[My5000](https://github.com/EgorTatarnikov/ForeignLanguages/blob/main/My5000.xls)** – мой личный словарный запас английского (не считая школьной базы), достаточный для чтения, видео и путешествий.
- **[Грамматика](https://github.com/EgorTatarnikov/ForeignLanguages/tree/main/English%20Grammar)** – mind map и заметки, систематизирующие базовую грамматику.

## 🛠 Cтек
Python · NLP · C# · Unity · C++ · C++Builder

## 🔗 Полезные ресурсы
- **[lingvolive.com](https://www.lingvolive.com/)** – онлайн словарь с правильным произношением и примерами из текстов
- **[context.reverso.net](https://context.reverso.net/)** – примеры использования слов и фраз в разных источниках с переводом с учетом контекста
- **[forvo.com](https://forvo.com/)** – база аудиофайлов с произношением слов, записанных разными людьми на множестве различных языков мира
- **Murphy Grammar in Use** – отличная серия книг по английской грамматике. Делится на 3 уровня. По моему мнению, синяя (средняя) книга охватывает всё, что нужно для свободного общения и понимания англоязычной информации
